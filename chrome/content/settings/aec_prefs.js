...
function enableField(aCheckbox, fieldID) {
  ...
  if (!field) return;
  if ((aCheckbox.localName == "radio" && aCheckbox.selected) || (aCheckbox
      .localName == "checkbox" && aCheckbox.checked)) {
...
function browseForFolder(pref_el_id) {
  ...
  var windowTitle = document.getElementById("aestrbundle").getString(
...
function showDetachWarning(radiobox) {
  ...
  var amessage = document.getElementById("aestrbundle").getString(
...
function showReturnReceiptSettings() {
  document.documentElement.openSubDialog(
    "chrome://attachmentextractor_cont/content/settings/aec_prefs_post_receipt.xul",
    "", null);
}
...
function showReportSettings() {
  document.documentElement.openSubDialog(
    "chrome://attachmentextractor_cont/content/settings/aec_prefs_post_report.xul",
    "", null);
}

function showDateSettings() {
  document.documentElement.openSubDialog(
    "chrome://attachmentextractor_cont/content/settings/aec_prefs_filename_date.xul",
    "", null);
...
function appendPrefEntry(listbox, prefid, emptytext) {
  ...
  var tb2idfix = listbox.itemCount ? null : listbox.id + listbox.childNodes
  ...
  entry.setAttribute("onsyncfrompreference", "var l=syncFromFilePref('" +
...
function fillParentFolderList(folderlist) {
  ...
  var emptytext = document.getElementById("aestrbundle").getString(
    "EmptyLineText");
...
function addEntry(button, folderlist) {
  ...
  if (folderlist.getItemAtIndex(folderlist.childNodes.length - 1).getAttribute(
      'preference') == prefid) {
    var clearpref = folderlist.getAttribute("preference") + "_" + (folderlist
      .childNodes.length + 1);
...
function check_filenamepattern(element, countpattern) {
  if ((!countpattern && filemaker.isValidFilenamePattern(element.value)) || (
      filemaker.isValidCountPattern(element.value))) return;
  var Cc = Components.classes;
  var Ci = Components.interfaces;
  var prompts = Cc["@mozilla.org/embedcomp/prompt-service;1"].getService(Ci
    .nsIPromptService);
  ...
  var fixed = (!countpattern) ? filemaker.fixFilenamePattern(element.value) :
...
function updateexamplefilename(fnpbox) {
  ...
  var docleansubject = document.getElementById('filenamepatterncleansubject')
    .checked;
  var exname = document.getElementById('filenamepattern_examplename').value;
  var cleansubjectstrings = document.getElementById('pref_cleansubjectstrings')
    .value.toLowerCase().split(',');

  var excache = new AttachmentFileMaker.AttachmentFileMakerCache();
  excache.subject = document.getElementById('filenamepattern_examplesubject')
    .value.replace(filemaker.tokenregexs.subject, "_");
  excache.author = document.getElementById('filenamepattern_exampleauthor')
    .value.replace(filemaker.tokenregexs.author, "");
  excache.authoremail = document.getElementById('filenamepattern_exampleauthor')
    .value.replace(filemaker.tokenregexs.authoremail, "");
  excache.datetime = filemaker.formatdatestring(datepattern, exampleDate);
  excache.mailfolder = document.getElementById('filenamepattern_examplefolder')
    .value.replace(filemaker.tokenregexs.folder, "");

  var cleansubject = filemaker.cleanSubjectLine(excache.subject,
    cleansubjectstrings);
  if (docleansubject) excache.subject = cleansubject;

  var st = filemaker.generate(pattern.replace(/#count#/g, ""), null, exname, 1,
    excache);
  var st2 = filemaker.generate(pattern.replace(/#count#/g, countpattern), null,
    exname, 1, excache);

  document.getElementById('filenamepattern_examplecleansubject').value =
    cleansubject;
  document.getElementById('filenamepattern_exampledategenerated').value =
    excache.datetime;
